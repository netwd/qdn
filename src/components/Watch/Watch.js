...
export class Watch extends React.Component {
  ...
  async componentDidMount() {
    ...
    if(data_level.data.subscription.level <= level) {
      ...
      this.setState({vidURL: data.data.url, loading: false, thumbnail: data_level.data.thumbnail, permission: true})
    } else {
    ...
    return (
      <div>
        {this.state.loading ? 
          ...
          <div>
              <div>
                <center><WatchContent videoId={this.state.vidURL} thumbnail={this.state.thumbnail}/></center>
              </div>
